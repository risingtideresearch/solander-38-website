...
export function Image(
  ...
  const assetWidth =
    "metadata" in src.asset ? src.asset.metadata?.dimensions?.width : undefined;
  const assetHeight =
    "metadata" in src.asset
  ...
  if (src.crop && assetWidth && assetHeight) {
    ...
    if (width && !height && assetWidth && assetHeight) {
      const croppedAspectRatio =
        (assetWidth * croppedWidthRatio) / (assetHeight * croppedHeightRatio);
  ...
  if (square) {
    ...
    // Use fit: 'crop' to maintain aspect ratio and crop to square
    imageUrl.size(size, size).fit("crop");
  } else {
    if (finalWidth) {
      imageUrl.width(
  ...
  if (finalHeight) {
    imageUrl.height(
      typeof finalHeight === "string" ? parseInt(finalHeight, 10) : finalHeight,
    );
  }

  if (!finalWidth || !finalHeight) {
    console.warn(
      "Image is missing width/height. Please provide dimensions or dereference asset in query.",
    );
  }
